...
function SortableItem({ item }: ItemProps) {
  ...
  return (
    <div
      className={clsx({ "shadow-2xs": isDragging })}
      ref={setNodeRef}
      style={style}
      {...attributes}
    >
...
## Drag-and-Drop Table using dnd-kit

DnD Kit is a modern, modular and lightweight drag and drop library that is highly compatible with the modern React ecosystem, and it works well with semantic <table> markup. Both of the official TanStack DnD examples, Column DnD and Row DnD, now use DnD Kit.
